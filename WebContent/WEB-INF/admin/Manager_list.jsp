			...
			<div id="page-inner">
				...
				<div class="panel panel-default">
					...
					<div class="panel-heading"></div>
					    ...
					    <form action="Manager" method='GET' id='listform'>
                			<input type="hidden" name="oper" value="listDeal"/>
			                <span class="panel-heading">关键字：</span>
			                <input type='search' name='searchName' id='searchName' placeholder="请输入要搜索的关键字" value="${searchName}"/>
			                <input type='submit' value='搜索' class="btn btn-primary"/>
			                <input type='button' class="btn btn-warning" onclick="javascript:location.href='Manager?oper=list'"  value='清空' />
                
                		</form>
                		
                		
					<div class="panel-body">
					<button onclick="javascript:location.href='Manager?oper=insert'"  class="btn btn-success">添加</button>   &nbsp;&nbsp;  <button onclick='javascript:datadel()' class="btn btn-success">批量删除</button>&nbsp;&nbsp;
						<div class="table-responsive">
								...
								<table class="table table-striped table-bordered table-hover" id="dataList">
					                 ...
					                 <tr>
					                 <td><input type='checkbox' id="line${bean.managerId}"  value='${bean.managerId}' /></td>
									 <td>${status.count}</td>
					                 <td>${bean.managerId}</td>
					                 <td>${bean.userName}</td>
					                 <td>${bean.userPass}</td>
					                 <td>
					                <a href="javascript:item_del(this,${bean.managerId});">删除</a>&nbsp;&nbsp;
								    <a href="Manager?oper=update&managerId=${bean.managerId}">修改</a>&nbsp;&nbsp;
								    <a href="Manager?oper=detail&managerId=${bean.managerId}">查看</a>     
					                 </td>
					                 </tr>
									</c:forEach>          
	...
	<script>
      ...
      function item_del(obj, id){
    	  ...
    	  if(confirm("确认要删除吗？")){
    		  $.ajax({
    			 type : 'POST',
    			 url : 'Manager?oper=deleteDeal&id='+id,
    			//dataType : 'json'
